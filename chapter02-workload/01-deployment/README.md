...
| 方式 | 速度 | 适用场景 |
|------|------|---------|
| 改文件 → `kubectl apply` | 慢，需要手动找回旧版本 | 有 git 版本控制、需要审计 |
| `kubectl rollout undo` | 秒级，旧 RS 直接扩容 | 紧急止血、不确定旧版本号 |

**`rollout undo` 的可靠性**：旧 RS 保留在 etcd 中（受 `revisionHistoryLimit` 控制，默认 10），undo 只是把 replicas=0 的旧 RS 重新扩容。不重拉镜像，秒级完成。

**但 `rollout undo` 不会回写磁盘文件**。集群重建后旧 RS 丢失。生产环境的正确做法：
- 紧急止血用 `rollout undo`
- 事后立刻 `kubectl get deploy <name> -o yaml > file.yaml` 导出覆盖到文件，git commit
- 日常更新严格走"改文件 → commit → apply"流程

### 7. `kubectl get deploy -o yaml` 导出的为什么和原始文件不一致？

导出的 YAML 包含三类原始文件没有的内容：

**运行时状态（可删）**：`uid`、`resourceVersion`、`creationTimestamp`、`annotations[last-applied-configuration]` 等
**默认值展开**：`revisionHistoryLimit: 10`、`progressDeadlineSeconds: 600`、`strategy.rollingUpdate.maxSurge: 25%`——你设写但 K8s 自动补了
**状态段 `status`**：当前副本数、更新进度等只读信息

导出的是 K8s 内部完整对象，原始文件是"最低要求声明"。两者用途不同——导出做调试，原始文件做版本管理。

### 8. 观察滚动更新的最佳方式

不是看 Pod 列表，而是看 ReplicaSet：

```bash
kubectl get rs -w
# 你会看到旧 RS 逐步缩容到 0，新 RS 逐步扩容到目标数
```

因为 Deployment 不直接管 Pod——它通过创建/修改 ReplicaSet 来驱动更新。`kubectl get rs -w` 是观察滚动更新过程的最佳窗口。
